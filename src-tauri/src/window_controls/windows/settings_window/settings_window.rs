...
impl SettingsWindow {
    pub fn new() -> Result<Self, tauri::Error> {
        ...
        if app_handle
            .get_window(&AppWindow::Settings.to_string())
            .is_none()
        {
            let window_builder = create_default_window_builder(&app_handle, AppWindow::Settings)?;
    ...
    pub fn start_event_listeners(settings_window: &Arc<Mutex<SettingsWindow>>) {
