...
use super::models::UpdateRefactoringSuggestionsMessage;

#[derive(Clone, Serialize, Deserialize, Debug, TS)]
#[serde(tag = "event", content = "payload")]
#[ts(export, export_to = "bindings/features/refactoring/")]
pub enum RefactoringEvent {
    UpdateRefactoringSuggestions(UpdateRefactoringSuggestionsMessage),
}

impl RefactoringEvent {
    pub fn publish_to_tauri(&self, app_handle: &tauri::AppHandle) {
        let event_name = ChannelList::RefactoringEvent.to_string();
