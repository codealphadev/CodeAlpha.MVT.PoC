<script lang="ts">
	...
	import {
		BORDER_WIDTH,
		compute_bracket_highlight_lines,
	...
	const update_brackets = async () => {
		...
		if (bracket_open.shapes[0] === undefined || bracket_close.shapes[0] === undefined) {
			return;
		}

		if (!is_rectangle(bracket_open.shapes[0]) || !is_rectangle(bracket_close.shapes[0])) {
			return;
		}
