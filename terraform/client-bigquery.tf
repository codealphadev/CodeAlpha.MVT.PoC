resource "google_bigquery_dataset" "client" {
  dataset_id                  = "client"
  location                    = local.region
}

resource "google_logging_project_sink" "client_logs" {
  name = "client-logs"
  destination = "bigquery.googleapis.com/projects/${google_project.project.project_id}/datasets/${google_bigquery_dataset.client.dataset_id}"
...
resource "google_bigquery_dataset_iam_binding" "client_logs" {
  dataset_id = google_bigquery_dataset.client.dataset_id
  role       = "roles/bigquery.dataEditor"

  members = [
    google_logging_project_sink.client_logs.writer_identity,
  ]
}
